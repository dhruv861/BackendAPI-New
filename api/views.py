...
def index(request):
    ...
    person = {"name": "Dhruv"}
    # for ex in data:
    #     Exercise.objects.create(name=ex["name"], bodyPart=ex["bodyPart"], target=ex["target"], equipment=ex["equipment"],
    #                              gifUrl=ex["gifUrl"])
...
class AllRooms(APIView):
    def get(self,request):
        ...
        # r= httpx.get("https://api.100ms.live/v2/room-codes/room/:room_id" ,headers={"Authorization": f'Bearer {token}'})
        # # print(r.text)
